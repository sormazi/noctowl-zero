...
export const MissionSection = () => {
  ...
  useEffect(() => {
    ...
    ctx = gsap.context(() => {
      ...
      ScrollTrigger.getAll().forEach(trigger => {
        if ((trigger as any)?.trigger === sectionRef.current) {
          trigger.kill();
        }
      });

      // Use pinType = 'fixed' for more stable pinning, and pin spacing off
      ScrollTrigger.create({
        ...
        pin: true,
        pinSpacing: true,
        pinType: "fixed", // Ensures true fixation; more reliable in modern browsers
        anticipatePin: 0.8,
        scrub: false,
      });
  ...
  return (
    <AnimatedSection as="section" id="mission" className="w-full mt-24 relative" delay={150}>
      {/* The sectionRef wrapper is ALWAYS full width and pinned */}
      <div ref={sectionRef} className="w-full relative left-0 right-0">
        {/* Keep content centered using an inner wrapper */}
